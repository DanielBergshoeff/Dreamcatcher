...
public class FreeLookCam : PivotBasedCameraRig
{
    ...
    protected override void FollowTarget(float deltaTime) {
        if (m_Target == null || RotatingAroundPillar) return;

        transform.position = Vector3.Lerp(transform.position, m_Target.position, deltaTime * (m_MoveSpeed + BonusMoveSpeed));
    }


    private void HandleRotationMovement() {
        if (Time.timeScale < float.Epsilon || RotatingAroundPillar) {
            return;
        }
        ...
        if (!resetRot) {
            ...
            m_PivotTargetRot = Quaternion.Euler(0f, 180f, 0f);
        }
